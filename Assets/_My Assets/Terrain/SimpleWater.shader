...
Shader "LowPoly/SimpleWater"
{
	Properties
	{
		...
		_NormalTriShift("Tri-Normal Shifting", Float) = 1
		...
		_WavesSpeed("WavesSpeed", Float) = 8.87
	...
	SubShader
	{
		...
		//#pragma target 4.6
		#pragma surface surf StandardSpecular addshadow vertex:vertexDataFunc //tessellate:tessFunction 
		struct Input
		{
			float2 uv_texcoord;
			float4 screenPos;
		...
		{
			float3 ase_vertex3Pos = float3(v.vertex.x,0,v.vertex.z);
			float3 world_Pos = mul((float4x4)unity_ObjectToWorld, float4(ase_vertex3Pos,1)).xyz;
			float3 ase_vertexNormal = v.normal.xyz;
			float3 ase_worldUp = float3(0,1,0);

			v.vertex.xyz = ase_vertex3Pos + ((sin(((_WavesAmount * world_Pos.z) + _Time.y * _WavesSpeed)) * ase_worldUp) * _WavesAmplitude);
			float3 waveNormalVec = _WavesAmplitude * _WavesAmount* (cos(((_WavesAmount * world_Pos.z) + _Time.y* _WavesSpeed)) * float3(0,0,-1));

			float3 noYnormal = normalize(ase_vertexNormal - float3(0, v.normal.y, 0));
			float phaseShift = acos(noYnormal.x) * sign(noYnormal.z);

			float3 triShiftNormalVec = sin((_Time.y - phaseShift) * _NormalTriShift) * float3(1, 0, 0)
				+ cos((_Time.y - phaseShift) * _NormalTriShift) * float3(0, 0, 1);
			v.normal.xyz = triShiftNormalVec + ase_worldUp / _NormalScale;//(ase_vertexNormal *0.2 + waveNormalVec + triShiftNormalVec) * _NormalScale + ase_worldUp;
		}
